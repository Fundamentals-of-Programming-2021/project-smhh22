...
//	SDL_SetRenderDrawColor(RENDERER, 0xff, 0xff, 0xff, 0xff);
	...
	CELL_WIDTH = (SCREEN_WIDTH - WINDOW_PADDING_RIGHT - WINDOW_PADDING_LEFT) / GRID_WIDTH;
	CELL_HEIGHT = (SCREEN_HEIGHT - WINDOW_PADDING_UP - WINDOW_PADDING_DOWN) / GRID_HEIGHT;
	...
	for (int i = 0; i < GRID_WIDTH; i++) {
		...
		for (int j = 0; j < GRID_HEIGHT; j++) {
			...
			boxColor(RENDERER, GRID[i][j].x1, GRID[i][j].y1, GRID[i][j].x2, GRID[i][j].y2, 0xff0000ff);
		}
	}
	uint32_t color = 0xffffff00;
	boxColor(RENDERER, 0, 0, SCREEN_WIDTH, WINDOW_PADDING_UP, color);
